...
class CustomLoggingCallback(BaseCallback):
    def __init__(self, env: ExplorationEnv, verbose=0):
        ...
        self.episode_mean_reward = 0.0
        self.known_map_percentage = 0.0
        self.done_data = 0.0
        self.truncated_data = 0.0
    ...
    def _on_step(self) -> bool:
        ...
        if done or truncated:
            self.episode_mean_reward = self.cumulated_reward / self.episode_steps
            self.episodes += 1
            self.known_map_percentage = self.env.unwrapped.get_known_map_percentage()
            if done:
                self.done_data = 1.0
            if truncated:    
                self.truncated_data = 1.0
            ...
            custom_data = {
                ...
                'mean_reward': self.episode_mean_reward,
                'known_map_percentage': self.known_map_percentage,
                'done': self.done_data,
                'truncated': self.truncated_data
            }
...
class Agent(Node):
    ...
    def train(self):
        ...
        model = PPO("MultiInputPolicy", self.env,
            ...
            sde_sample_freq=4,
            verbose=1)
        callbacks = [wand_cb, CustomLoggingCallback(self.env), SaveModelByEpisodeCallback(save_freq=1000, save_path=f"{self.save_model_path}/{run.name}")]
        try:
